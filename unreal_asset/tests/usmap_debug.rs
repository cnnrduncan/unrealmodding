...
use unreal_asset_base::unversioned::{EUsmapVersion, EUsmapCompressionMethod};
...
fn debug_oblivion_remastered_usmap() {
    ...
    println!("Version byte: {} (position: {})", usmap_version_byte, cursor.position());
    ...
    if is_ue4ss_format {
        ...
        if has_versioning {
            ...
            println!("Versioning flag byte: 0x{:02X} -> has_versioning: {} (position: {})", 
                     versioning_flag, has_versioning, cursor.position());
        }
        
        if has_versioning {
            let object_version = cursor.read_i32::<LE>().expect("Failed to read object version");
            println!("Object version: {} (position: {})", object_version, cursor.position());
            
            let object_version_ue5 = cursor.read_i32::<LE>().expect("Failed to read UE5 version");
            println!("Object version UE5: {} (position: {})", object_version_ue5, cursor.position());
            
            // Skip custom versions array for now - this is complex
            let custom_versions_len = cursor.read_i32::<LE>().expect("Failed to read custom versions length");
            println!("Custom versions length: {} (position: {})", custom_versions_len, cursor.position());
        ...
        let compression = cursor.read_u8().expect("Failed to read compression");
        println!("Compression method: {} (position: {})", compression, cursor.position());
    ...
    let compressed_size = cursor.read_u32::<LE>().expect("Failed to read compressed size");
    println!("Compressed size: {} (position: {}, read from: {})", compressed_size, cursor.position(), pos_before_sizes);
    
    let decompressed_size = cursor.read_u32::<LE>().expect("Failed to read decompressed size");
    println!("Decompressed size: {} (position: {})", decompressed_size, cursor.position());
    ...
    println!("Bytes interpreted as compressed size (starting at position {}):", pos_before_sizes);
    for i in 0..8 {
