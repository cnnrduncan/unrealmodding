...
fn comprehensive_oblivion_usmap_test() {
    ...
    let usmap = Usmap::new(cursor)
        .expect("Failed to parse OblivionRemastered-5.3.2-.usmap file");
    ...
    println!("     Average name length: {:.1} chars", total_name_chars as f64 / usmap.name_map.len() as f64);
    ...
    for (_, enum_name, enum_values) in usmap.enum_map.iter() {
        ...
        if enum_values.len() > 100 {
            large_enums += 1;
            if large_enums <= 3 {
                println!("   📊 Large enum {}: {} values", enum_name, enum_values.len());
            }
        }
    ...
    println!("     Average values per enum: {:.1}", total_enum_values as f64 / usmap.enum_map.len() as f64);
    ...
    for (_, schema_name, schema) in usmap.schemas.iter() {
        ...
        if schema.properties.len() > 50 {
            large_schemas += 1;
            if large_schemas <= 3 {
                println!("   📊 Large schema {}: {} properties", schema_name, schema.properties.len());
            }
        }
        ...
        if !schema.super_type.is_empty() {
            ...
            if !usmap.schemas.get_by_key(&schema.super_type).is_some() {
                ...
                if orphaned_schemas <= 5 {
                    println!("   ⚠️  Schema {} has non-existent super type: {}", schema_name, schema.super_type);
                }
            }
        }
        ...
        if schema.prop_count as usize != schema.properties.len() {
            println!("   ⚠️  Schema {} prop_count mismatch: declared={}, actual={}", 
                     schema_name, schema.prop_count, schema.properties.len());
        }
        ...
        for (_, (prop_name, dup_index), _property) in &schema.properties {
            let entry = property_names.entry(prop_name.clone()).or_insert(Vec::new());
            entry.push(*dup_index);
        }
        
        for (prop_name, dup_indices) in property_names {
            if dup_indices.len() > 1 {
                println!("   📊 Schema {} has property {} with {} duplicates: {:?}", 
                         schema_name, prop_name, dup_indices.len(), dup_indices);
            }
        }
    }
    ...
    println!("     Orphaned schemas (invalid super): {}", orphaned_schemas);
    println!("     Total properties: {}", total_properties);
    println!("     Average properties per schema: {:.1}", total_properties as f64 / usmap.schemas.len() as f64);
    ...
    for (_, schema_name, schema) in &test_schemas {
        ...
        for (_, (prop_name, dup_index), _) in schema.properties.iter().take(3) {
            match schema.get_property(prop_name, *dup_index) {
                ...
                None => {
                    failed_lookups += 1;
                    println!("   ⚠️  Failed to lookup property {} in schema {}", prop_name, schema_name);
                }
            }
        }
    }
    ...
    for (_, schema_name, schema) in usmap.schemas.iter().take(20) {
        ...
        if all_properties.len() > schema.properties.len() {
            ...
            if inheritance_chains_found <= 5 {
                println!("   📊 Schema {} inherits: own={}, total={}", 
                         schema_name, schema.properties.len(), all_properties.len());
            }
        }
    }
    ...
    println!("     Inheritance chains found: {}", inheritance_chains_found);
    ...
    let common_types = [
        ...
        "FString", "FName", "TSoftObjectPtr", "TArray", "TMap", "TSet",
        "UObject", "AActor", "UComponent", "UStaticMesh", "UTexture"
    ];
    ...
    println!("   Common types found: {}/{}", found_types, common_types.len());
    
    // Test 7: Memory usage estimation
    println!("\n📊 Memory usage estimation:");
    let name_map_size = usmap.name_map.iter().map(|s| s.len()).sum::<usize>();
    let enum_map_size = usmap.enum_map.iter()
        .map(|(_, name, values)| name.len() + values.iter().map(|v| v.len()).sum::<usize>())
        .sum::<usize>();
    let schema_map_size = usmap.schemas.iter()
        .map(|(_, name, schema)| {
            name.len() + schema.super_type.len() + 
            schema.properties.iter().map(|(_, (prop_name, _), _)| prop_name.len()).sum::<usize>()
        })
        .sum::<usize>();
    ...
    println!("   Total string data: ~{} KB", (name_map_size + enum_map_size + schema_map_size) / 1024);
    
    // Final validation
    println!("\n✅ Comprehensive test completed!");
    println!("🎯 Summary:");
    println!("   • File parsing: SUCCESS");
    println!("   • Name map: {} entries, {} empty", usmap.name_map.len(), empty_names);
    println!("   • Enum map: {} enums, {} values total", usmap.enum_map.len(), total_enum_values);
    println!("   • Schema map: {} schemas, {} properties total", usmap.schemas.len(), total_properties);
    println!("   • Property lookups: {}/{} successful", successful_lookups, successful_lookups + failed_lookups);
    println!("   • Memory usage: ~{} KB", (name_map_size + enum_map_size + schema_map_size) / 1024);
    
    // Assert critical conditions
    assert!(usmap.name_map.len() > 1000, "Should have substantial name map");
    assert!(usmap.enum_map.len() > 100, "Should have substantial enum map");
    assert!(usmap.schemas.len() > 1000, "Should have substantial schema map");
    assert!(failed_lookups == 0, "All property lookups should succeed");
    assert!(empty_names < usmap.name_map.len() / 10, "Too many empty names");
