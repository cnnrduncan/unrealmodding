...
impl Usmap {
    ...
    /// Parse usmap file
    pub fn parse_data<C: Read + Seek>(&mut self, mut cursor: C) -> Result<(), Error> {
        // First, try to determine if this is a variant format by reading the header
        let mut header_buffer = [0u8; 32]; // Read enough bytes to analyze the header
        let bytes_read = cursor.read(&mut header_buffer)?;
        if bytes_read < 11 {
            return Err(Error::invalid_file("File too small to be a valid usmap".to_string()));
        }
        ...
        // Check magic
        let magic = u16::from_le_bytes([header_buffer[0], header_buffer[1]]);
        ...
        let version_byte = header_buffer[2];
        let is_ue4ss_format = version_byte == 0;

        // Try to detect if this is a variant format by checking if sizes make sense
        let mut use_variant_parsing = false;
        if !is_ue4ss_format {
            // For non-UE4SS files, check if standard parsing would work
            let mut test_pos = 3; // After magic + version
            
            // Skip versioning fields if present
            let usmap_version = EUsmapVersion::try_from(version_byte).map_err(|_| {
                Error::invalid_file("Invalid usmap version".to_string())
            })?;
            
            if usmap_version >= EUsmapVersion::PackageVersioning {
                let has_versioning = header_buffer[3] != 0;
                test_pos += 1;
                if has_versioning {
                    // Skip version fields: object_version (4) + ue5_version (4) + custom_versions_len (4)
                    test_pos += 12;
                    let _custom_versions_len = u32::from_le_bytes([
                        header_buffer[test_pos - 4],
                        header_buffer[test_pos - 3], 
                        header_buffer[test_pos - 2],
                        header_buffer[test_pos - 1]
                    ]);
                    // This would be too complex to calculate accurately in the header, so skip for variant detection
                }
            }
            
            // Try compression + sizes at standard position
            if test_pos + 9 <= bytes_read {
                test_pos += 1; // compression byte
                let compressed_size = u32::from_le_bytes([
                    header_buffer[test_pos],
                    header_buffer[test_pos + 1],
                    header_buffer[test_pos + 2], 
                    header_buffer[test_pos + 3]
                ]);
                
                // If the size is unreasonably large, try variant parsing
                if compressed_size > 10_000_000 {
                    use_variant_parsing = true;
                }
            }
        }

        if use_variant_parsing {
            self.parse_with_variant_layout(cursor)
        } else {
            self.parse_with_standard_layout(cursor)
        }
    }

    /// Parse with standard usmap format
    fn parse_with_standard_layout<C: Read + Seek>(&mut self, cursor: C) -> Result<(), Error> {
        ...
        self.parse_compressed_data(reader, compressed_size, decompressed_size, usmap_version, is_ue4ss_format)
    }

    /// Parse with variant header layout (e.g., OblivionRemastered format)
    fn parse_with_variant_layout<C: Read + Seek>(&mut self, cursor: C) -> Result<(), Error> {
        let mut reader = RawReader::<PackageIndex, C>::new(
            Chain::new(cursor, None),
            ObjectVersion::UNKNOWN,
            ObjectVersionUE5::UNKNOWN,
        ...
        if is_ue4ss_format {
            // UE4SS format should work with standard parsing
            return Err(Error::invalid_file(
                "UE4SS format should be handled by standard parser".to_string(),
            ));
        }

        // For non-UE4SS format, try different header layouts
        // Skip potential extra bytes and find the correct position for sizes
        let data_length = reader.data_length()?;
        let mut found_valid_layout = false;
        let mut compressed_size = 0u32;
        let mut decompressed_size = 0u32;

        // Try different offsets for the size fields (after magic + version)
        for skip_bytes in 1..8 {
            let test_position = 3 + skip_bytes; // After magic (2) + version (1)
            
            if test_position + 8 <= data_length {
                reader.seek(std::io::SeekFrom::Start(test_position))?;
                
                // Try reading compression method (should be 0-4)
                let compression_byte = reader.read_u8()?;
                if compression_byte <= 4 {
                    let test_compressed_size = reader.read_u32::<LE>()?;
                    let test_decompressed_size = reader.read_u32::<LE>()?;
                    let remaining_bytes = data_length - reader.position();
                    
                    // Check if this layout makes sense
                    if test_compressed_size as u64 <= remaining_bytes && 
                       test_compressed_size > 0 && 
                       test_compressed_size < 10_000_000 && // Reasonable size limit
                       test_decompressed_size > 0 &&
                       test_decompressed_size < 100_000_000 {
                        
                        self.compression_method = EUsmapCompressionMethod::try_from(compression_byte)?;
                        compressed_size = test_compressed_size;
                        decompressed_size = test_decompressed_size;
                        found_valid_layout = true;
                        break;
                    }
                }
            }
        ...
        if !found_valid_layout {
            return Err(Error::invalid_file(
                "Could not determine usmap header layout".to_string(),
            ));
        }
    ...
    /// Parse the compressed data section (shared between standard and variant parsing)
    fn parse_compressed_data<C: Read + Seek>(
        &mut self, 
        mut reader: RawReader<PackageIndex, C>, 
        compressed_size: u32, 
        decompressed_size: u32, 
        usmap_version: EUsmapVersion, 
        is_ue4ss_format: bool
    ) -> Result<(), Error> {
